...
Octoblob no longer maintains backward compatibility with Python 2.7, and now requires Python 3, along with several libraries from the Python 3 "scientific stack", viz. [NumPy](https://numpy.org), [SciPy](https://scipy.org), and [matplotlib](https://matplotlib.org). In Windows and MacOS, the simplest way to install the stack is to use one of the scientific Python distributions, such as [Anaconda](https://anaconda.com) or [Enthought Python Distribution](https://enthought.com). These distributions are available for Linux as well, or the stack can be installed and kept up to date using your package manager (apt, snap, yum, etc.) just as easily. When the documentation requires instructions for installing or modifying packages, the Anaconda instructions will be given, as it is the most popular route.

## Environmental variables

Generally speaking, you should have one folder on your computer where you store Python modules--both those that you create and those that you download. As a convention, on the machines in our lab we always use ```C:\code```, but anywhere is fine. Look at the environmental variables on your machine, and see if ```PYTHONPATH``` is defined. If it is, and ```C:\code``` isn't part of it, add it to the path (using the correct delimiter, e.g. ';', for your system). If it's not defined, then create a new envornment variable ```PYTHONPATH``` and set it to ```C:\code```. On Linux machines we use ```~/code```. If you choose another location, for instructions requiring ```PYTHONPATH```, make sure you use your preferred location instead of the defaults used in the instructions.
...
The first step in constructing a script is to import the tools you'll need. ```numpy``` and ```matplotlib``` are the standard numerical and plotting libraries in Python, and are always imported.
